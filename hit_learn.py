...
def target_policy(env, obs):
    targetpos = [[0.26,0.15,0.08], [0.35, -0.2, 0.4], [0.28,-0.24,0.08]]
    nowpos = p.getLinkState(env.robot_id, env.end_eff_idx)
    if obs[-1] == 1:
        _target = targetpos[0]
        target_theta = p.calculateInverseKinematics(env.robot_id, env.end_eff_idx, _target)
    elif obs[-1] == 2 and nowpos[0][1] > -0.1:
        _target = targetpos[1]
        target_theta = p.calculateInverseKinematics(env.robot_id, env.end_eff_idx, _target, targetOrientation=[1.57,0,0])
    else:
        _target = targetpos[2]
        target_theta = p.calculateInverseKinematics(env.robot_id, env.end_eff_idx, _target)
    ...
    distance_reward = 0.25*np.exp(-sum([i**2 for i  in delta_theta])/len(delta_theta))
    return target_velocity, distance_reward

def mimic(target_action, action):
    dist = 0
    for i in range(len(action)):
        dist += np.power((target_action[i] - action[i])/2, 2)
    return 0.5*np.exp(-dist/len(action))

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--seed", default=2, type=int)                  # Sets Gym, PyTorch and Numpy seeds
    parser.add_argument("--start_episode_num", default=1e8, type=int)     # How many time steps purely random policy is run for
    parser.add_argument("--max_episode_num", default=1e12, type=float)     # Max time steps to run environment for
    parser.add_argument("--discount", default=0.99, type=float)         # Discount factor
    parser.add_argument("--max_expl_noise", default=1.5, type=float)    # Std of Gaussian exploration noise
    ...
    while episode_num < args.max_episode_num:
        if done:
            ...
            if total_timesteps != 0:
                policy.train(replay_buffer, int(episode_timesteps/5), total_timesteps, args.batch_size, args.discount, args.tau, args.policy_noise, args.noise_clip, args.policy_freq)
